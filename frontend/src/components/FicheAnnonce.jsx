...
import { ReportAPI, ListingAPI } from "../services/api";
  ...
  return (
    <div className="card p-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="font-tag text-[11px] text-neutral-500">
            {seller?.neighborhood || seller?.bio || ""}
          </p>
        </div>
      </div>
      ...
      <p className="mt-3 font-tag text-[11px] text-neutral-500">
        Membre depuis {formatMemberSince(seller?.createdAt)}
      </p>
...
export default function FicheAnnonce({ listing, seller, reviews, averageRating, categories, neighborhoods, loading, error }) {
  ...
  useEffect(() => {
    ...
    ListingAPI.patch(listing.id, { views: (listing.views || 0) + 1 }).catch(() => {});
  }, [listing?.id]);

  const categoryName = categories?.find((c) => c.id === listing?.categoryId)?.name || null;
  const neighborhoodName = neighborhoods?.find((n) => n.id === listing?.neighborhoodId)?.name || null;
  ...
  async function handleReport(e) {
    ...
    setReportError(null);
    try {
      await ReportAPI.create({ listingId: listing.id, reason: reportReason.trim() });
      setReportSent(true);
    } catch {
      ...
      <div className="container-shell py-6 md:py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          <div className="space-y-6">
            <GaleriePhotos images={listing.images} title={listing.title} />
            ...
            <div className="card p-5">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-neutral-500">
                ...
                {neighborhoodName && <span className="h-1 w-1 rounded-full bg-neutral-300" />}
